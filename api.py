...
from flask import Flask, Response, request, json, abort, render_template
...
def base():
    return Response(
        response=json.dumps({'Status': 'Up and running!'}), 
        status=200, 
...
def insert():
    if request.json is None or request.json == {}:
        return Response(
            response=json.dumps({'Error': 'Please provide correct/complete information'}),
            status=400,
            mimetype='application/json')
...
def update(id):
    if request.json is None or request.json == {}:
        return Response(
            response=json.dumps({'Error': 'Please provide data to update'}),
            status=400,
            mimetype='application/json')
    elif id < 0:
        return Response(
            response=json.dumps({'Error': 'Please provide correct tab id'}),
            status=400,
            mimetype='application/json')
...
def delete(id):
    if id < 0:
        return Response(
            response=json.dumps({'Error': 'Please provide correct item id'}),
            status=400,
            mimetype='application/json')
